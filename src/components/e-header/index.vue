...
export default {
	...
	methods: {
		...
		handleUser(name: string): void {
			switch (name) {
				case 'logout':
					this.$store.commit('common/setUser', null)
		...
		handleLink(url: string, title: string): void {
			this.$store.commit('common/setNavIndex', url)
			document.title = title
			this.$router.push(url)
		},
	},
	mounted(): void {
		this.$store.commit('common/setNavIndex', this.$route.path)
		detail()
			.then(res => {
				this.$store.commit('common/setUser', res)
			})
			.catch(() => {
				this.$store.commit('common/user', null)
				localStorage.removeItem('cakev-login')
